...
export async function uploadToCloudinary(
  ...
  const result = await cloudinary.uploader.upload(input, {
    ...
    tags: options.tags,
    // quality/fetch_format are delivery-time transformation parameters, not
    // valid bare top-level upload options — Cloudinary silently ignores them
    // there. Passing them inside `transformation` applies them as an eager
    // transformation at upload time; buildImageUrl() below applies the same
    // optimization again at delivery time for any URL built ad hoc.
    transformation: {
      quality: 'auto',
      fetch_format: 'auto',
