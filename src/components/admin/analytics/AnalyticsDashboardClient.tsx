...
import { AdminCard, Btn } from '@/components/admin/shared/ui'
...
export function AnalyticsDashboardClient({ summary }: { summary: AnalyticsSummary }) {
  ...
  const refresh = async () => {
    setRefreshing(true)
    try {
      const res = await fetch('/api/admin/analytics/refresh', { method: 'POST' })
      if (res.ok) {
    ...
    <div>
      <div className="flex justify-between items-center mb-5">
        <p className="text-dimmer text-xs">
          Data updates hourly via scheduled refresh{lastRefreshed ? ` · last manual refresh ${lastRefreshed}` : ''}.
        </p>
